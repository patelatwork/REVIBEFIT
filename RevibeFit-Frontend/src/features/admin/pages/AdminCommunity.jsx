...
import { MessageSquare, Trophy, Users, Plus, TrendingUp } from 'lucide-react';
...
const AdminCommunity = () => {
  ...
  useEffect(() => { fetchPosts(); fetchStats(); }, [category, sort, page]);
  ...
  const fetchPosts = async () => {
    ...
    try {
      ...
      if (data.success) { setPosts(data.data.posts || []); setTotalPages(data.data.totalPages || 1); }
  ...
  const handlePostCreated = () => { setShowCreateModal(false); setPage(1); setCategory(''); fetchPosts(); fetchStats(); };
        ...
        <div className="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8 py-8 pt-20 lg:pt-8">
          ...
          <div className="flex items-center justify-between flex-wrap gap-4 mb-8">
            <div>
              ...
              <p className="text-gray-500 text-sm">Manage and engage with the fitness community.</p>
            </div>
            <div className="flex items-center gap-3">
              ...
              <Link to="/admin/community/challenges"
          ...
          <div className="grid grid-cols-3 gap-4 mb-8">
            {[
              ...
              <div key={s.label} className="bg-white rounded-xl border border-gray-100 p-4 flex items-center gap-3">
                <div className="p-2 bg-[#225533]/10 rounded-lg"><s.icon size={20} className="text-[#225533]" /></div>
                <div>
            ...
            <div className="flex items-center gap-1 bg-white rounded-xl border border-gray-100 p-1 flex-wrap">
              {categories.map((cat) => (
                <button key={cat.value} onClick={() => { setCategory(cat.value); setPage(1); }}
                  className={`px-3 py-1.5 rounded-lg text-xs font-medium transition-all ${category === cat.value ? 'bg-[#225533] text-white' : 'text-gray-600 hover:bg-gray-100'}`}>
                  {cat.icon} {cat.label}
                </button>
              ))}
            </div>
            ...
            <div className="space-y-4">
              {posts.map((post) => (
                <PostCard key={post._id} post={post}
                  ...
                  onPostClick={() => navigate(`/admin/community/post/${post._id}`)}
                  onReactionToggled={fetchPosts} onDeleted={fetchPosts} />
              ))}
            </div>
          )}
