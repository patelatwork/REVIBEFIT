...
import { ArrowLeft, Trophy, Calendar, Users, Target, Clock, Medal, Trash2, Plus } from 'lucide-react';
import TrainerNavbar from '../components/TrainerNavbar';

const API_URL = import.meta.env.VITE_API_URL || 'http://localhost:8000';

const TrainerChallengeDetail = () => {
  ...
  const deleteChallenge = async () => {
    ...
    try {
      ...
      navigate('/trainer/community/challenges');
    } catch {}
  };

  const currentUser = getCurrentUser();
  const isCreator = currentUser && challenge && (currentUser._id === (challenge.createdBy?._id || challenge.createdBy));
  const now = new Date();
            ...
            <div className="lg:col-span-2 space-y-6">
              <div className="bg-white rounded-xl border border-gray-100 overflow-hidden">
                {challenge.coverImage && (
                  <img src={challenge.coverImage.startsWith('http') ? challenge.coverImage : `${API_URL}/${challenge.coverImage}`}
                ...
                <div className="p-6">
                  <div className="flex items-start justify-between gap-4">
                    <div>
                      ...
                      <div className="flex items-center gap-2 flex-wrap">
                        ...
                        <span className={`px-2.5 py-0.5 rounded-full text-xs font-medium ${
                          ...
                          challenge.difficulty === 'intermediate' ? 'bg-yellow-100 text-yellow-700' : 'bg-red-100 text-red-700'
                    ...
                    {isCreator && <button onClick={deleteChallenge} className="p-2 text-gray-400 hover:text-red-500"><Trash2 size={18} /></button>}
                  </div>
                  ...
                  <div className="space-y-2">
                    {leaderboard.map((entry, i) => (
                      <div key={entry._id} className={`flex items-center gap-3 px-4 py-3 rounded-xl ${i < 3 ? 'bg-[#225533]/5' : 'bg-gray-50'}`}>
                        ...
                        <div className="w-8 h-8 rounded-full bg-[#225533] flex items-center justify-center text-white text-xs font-bold">
...
export default TrainerChallengeDetail;
