      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        <div className="flex items-center justify-between flex-wrap gap-4 mb-8">
          <div>
            ...
            <p className="text-gray-500 text-sm">Create and manage challenges to motivate the community.</p>
          </div>
          <Link
            to="/trainer/community/challenge/create"
          ...
          {statusTabs.map((tab) => (
            <button
              ...
              className={`px-4 py-2 rounded-lg text-sm font-medium transition-all ${
                status === tab.value ? 'bg-[#225533] text-white' : 'bg-white text-gray-600 hover:bg-gray-100 border border-gray-200'
              }`}
            >
              {tab.label}
            </button>
        ...
        {loading ? (
          ...
          <div className="text-center py-20 bg-white rounded-xl border border-gray-100">
            ...
            <Link to="/trainer/community/challenge/create" className="inline-flex items-center gap-2 px-5 py-2.5 bg-[#225533] text-white rounded-xl font-medium hover:bg-[#3f8554] transition-colors">
              <Plus size={18} /> Create Challenge
            </Link>
          </div>
        ) : (
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {challenges.map((ch) => (
              <ChallengeCard key={ch._id} challenge={ch} basePath="/trainer/community" hideJoin={true} />
            ))}
          </div>
        )}
...
export default TrainerChallenges;
