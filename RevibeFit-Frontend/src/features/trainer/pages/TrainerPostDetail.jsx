...
const TrainerPostDetail = () => {
  ...
  const checkFollowing = async (userId) => {
    try {
      const token = getToken();
      const res = await fetch(`${API_URL}/api/community/is-following/${userId}`, {
  ...
  const toggleFollow = async () => {
    ...
    try {
      const token = getToken();
      const res = await fetch(`${API_URL}/api/community/follow/${authorId}`, {
  ...
  const toggleReaction = async (type) => {
    try {
      ...
      const res = await fetch(`${API_URL}/api/community/react`, {
        ...
        credentials: 'include',
        body: JSON.stringify({ targetId: post._id, targetType: 'post', type }),
      });
      const data = await res.json();
  ...
  const deletePost = async () => {
    ...
    try {
      const token = getToken();
      await fetch(`${API_URL}/api/community/posts/${post._id}`, {
        ...
        {loading ? (
          ...
          <div className="bg-white rounded-xl border border-gray-100 overflow-hidden">
            ...
            <div className="p-6 pb-0">
              ...
              {post.category && (
                <span className="inline-block mt-3 px-3 py-1 bg-[#225533]/10 text-[#225533] rounded-full text-xs font-medium capitalize">
              ...
              {post.images?.length > 0 && (
                <div className={`mt-4 grid gap-2 ${post.images.length === 1 ? 'grid-cols-1' : 'grid-cols-2'}`}>
                  {post.images.map((img, i) => (
                    <img key={i} src={img.startsWith('http') ? img : `${API_URL}/${img}`} alt="" className="rounded-lg w-full object-cover max-h-96" />
                  ))}
                </div>
              )}
              {post.tags?.length > 0 && (
            ...
            <div className="px-6 py-3 border-t border-gray-100 flex items-center gap-4">
              <div className="relative">
                <button
                  ...
                  className="flex items-center gap-1.5 text-gray-500 hover:text-[#225533] text-sm transition-colors"
                >
                ...
                {showReactions && (
                  <div className="absolute bottom-full left-0 mb-2 flex gap-1 bg-white border border-gray-200 rounded-full px-2 py-1 shadow-lg">
                    {Object.entries(reactionEmojis).map(([type, emoji]) => (
                      <button key={type} onClick={() => toggleReaction(type)} className="text-xl hover:scale-125 transition-transform px-1"
...
export default TrainerPostDetail;
